...
- `results/m0/`, `results/summary/m0.json`, `results/summary/m0b.json`은 **불변**이다. 스크립트의 기본 출력 경로도 옛 것 그대로 두고, M0c 실행은 명시 경로(`results/m0c/…`, `results/summary/m0c.json`)로만 쓴다(D.5). 기존 real-data 테스트는 손대지 않는다.
...
- 희소성 스크립트의 grid 행에 `kc_kc_scale` 키와 `mbon_hz_rest_trimmed_per_seed`가 들어간다. 옛 `results/m0/sparsity.json` 행에는 두 키가 없다 — 행 선택은 `g.get("kc_kc_scale", 1.0)`으로 옛 행을 1.0 엔진으로 읽는다.
...
| `flymon/brain/pool_bench.py` | `m0c_gate` |
...
| `scripts/write_m0c_summary.py` | `results/summary/m0c.json` 합성(`pick_row`, `compose`) |
...
| `tests/test_write_m0c_summary.py`, `tests/test_summary.py` | Task 3 테스트(작성기 단위, m0c.json 검사) |
| `README.md`, 스펙 6절 | Task 4 문서 |
...
파일 머리의 import를 다음으로 바꾼다(`dataclasses` 추가).
...
def build_csc(conn: Connectome, params: Params, apl_idx: np.ndarray, kc_idx: np.ndarray) -> CSC:
    ...
    m = is_apl[pre]
    mv[m] *= np.float32(params.apl_scale)
    if params.kc_kc_scale not in (0.0, 1.0):
        m = kc_kc[keep]
...
Run: `uv run pytest -q tests/brain/test_connectome.py tests/brain/test_config.py tests/brain/test_shuffle_kc_mbon.py`
Expected: PASS (연결체 10개, 설정 3개, 셔플 전부).
...
import에 `m0c_gate`를 추가하고(`m0b_gate, m0c_gate,`), 파일 끝에:
...
Expected: FAIL — `ImportError: cannot import name 'odor_runaway_job'`, `cannot import name 'm0c_gate'`.
...
- [ ] **Step 6: 구현 — `flymon/brain/pool_bench.py` 끝에**
...
def m0c_gate(sparsity_row: dict, baseline: dict, runaway: dict, equivalence: dict, budget: dict,
    ...
    and runaway and equivalence and throughput; the conditioning criterion is recorded, not gated on."""
    ...
    runaway_ok = max(runaway["rest_n_kc_over_sat_per_seed"]) == 0 and max(runaway["odor_B_n_kc_over_sat_per_seed"]) == 0
...
git commit -m "feat(brain): odour-window runaway job (150 Hz), per-seed trimmed baseline, m0c_gate (spec D.4)"
```
...
**Files:**
- Modify: `scripts/reproduce_flybrain_measurements.py`, `scripts/bench_pool.py`
- Create: `scripts/write_m0c_summary.py`
- Test: `tests/test_write_m0c_summary.py`(새 파일), `tests/test_summary.py`(끝에 m0c 검사 추가)
...
- Produces: CLI 옵션(README의 M0c 명령이 그대로 돈다); `write_m0c_summary.pick_row(grid, p) -> dict`, `compose(p, sp, old, new, reported, th, limit_hours=60.0) -> dict`; `results/summary/m0c.json` 스키마 = `compose`의 반환(키: `params_frozen, git_commit, kc_kc_scale_old_engine, seeds, sparsity, baseline, runaway, conditioning, conditioning_reported_seeds, equivalence, throughput, budget, memory, reproduce_wall_clock_s, generated_at, gate`).
...
def _row(**kw):
    r = {"kc_thresh": 1.5, "apl_scale": 0.1, "mbon_hold_frac": 0.85, "kc_kc_scale": 0.0, "frac_active_A": 0.064, "frac_active_B": 0.049,
...
def _reproduce(seeds, n_rest=8, n_odor=64, odor_over=0):
    ...
    return {"seeds": seeds, "conditioning": cond,
...
def _throughput():
    ...
    return {"rows": [{"workers": W, "n_flies_batch": W, "s_decide_batch_max": d, "s_reinforce_batch_max": r, "ms_decision_max": m, "ms_reinforce_max": m}
                     for W, d, r, m in ((4, 9.8, 2.8, 1.8), (8, 11.1, 3.5, 2.2), (16, 16.1, 5.3, 4.4))]}


def test_compose_records_every_gate_term_and_the_seed_policy():
    p = Params()
    out = w.compose(p, {"grid": [_row()]}, _reproduce(list(range(8))), _reproduce(list(range(8, 16))), _reproduce(list(range(8))), _throughput())
...
def test_compose_writes_a_failed_gate_instead_of_refusing():
    p = Params()
    out = w.compose(p, {"grid": [_row(mbon_hz_rest_trimmed=2.94)]}, _reproduce(list(range(8))),
...
def test_m0c_summary_records_the_pre_registered_gate():
    ...
    assert d["seeds"]["conditioning_judged"] == list(range(8, 16)) and len(d["seeds"]["rest"]) >= 8
    assert d["runaway"]["odor_B_n_seeds"] >= 64
...
+        grid.append({"kc_thresh": kc_thresh, "apl_scale": apl, "mbon_hold_frac": hold, "kc_kc_scale": a.kc_kc_scale, **mean, **rest})
...
@@ -100,7 +100,7 @@ def cmd_conditioning(a):
...
+                       da_baseline_ms=a.da_baseline_ms, kc_kc_scale=a.kc_kc_scale)
     ...
     viz = None
     if a.viz:
@@ -112,7 +112,7 @@ def cmd_conditioning(a):
...
+from flymon.brain.pool_bench import FLOAT_KEYS, budget_table, exact_match, m0b_gate, throughput_row
...
@@ -53,10 +53,11 @@ def cmd_throughput(a):
...
@@ -70,21 +71,28 @@ def _rss_by_pid(samples) -> dict:
 def cmd_reproduce(a):
     npz = _npz(a)
...
@@ -98,28 +106,46 @@ def cmd_reproduce(a):
...
- [ ] **Step 5: 구현 — `scripts/write_m0c_summary.py`(새 파일, 전문)**
...
def compose(p: Params, sp: dict, old: dict, new: dict, reported: dict | None, th: dict, limit_hours: float = 60.0) -> dict:
    ...
    out = {"params_frozen": dataclasses.asdict(p), "git_commit": _git_commit(), "kc_kc_scale_old_engine": 1.0,
           "seeds": {"sparsity": [100, 101, 102], "rest": row["rest_seeds"], "odor_runaway": [r["seed"] for r in new["odor_runaway"]["per_seed"]],
...
Run: `uv run pytest -q tests/test_write_m0c_summary.py tests/test_summary.py && uv run python scripts/bench_pool.py --help && uv run python scripts/reproduce_flybrain_measurements.py conditioning --help | grep -c "seed-start"`
Expected: 테스트 PASS(m0c.json 검사는 skip), `--help` 정상, `1`.
...
```bash
git add scripts/reproduce_flybrain_measurements.py scripts/bench_pool.py scripts/write_m0c_summary.py tests/test_write_m0c_summary.py tests/test_summary.py
git commit -m "feat(scripts): M0c runs — kc-kc-scale/seed-start, odour runaway check, arm equivalence, conditioning-out, write_m0c_summary"
```

---

### Task 4: 문서 — README M0c 절과 "우리가 정한 것(M0c)", 스펙 6절
...
3. `uv run python scripts/bench_pool.py reproduce --seed-start 8 --rest-seeds 8 --odor-runaway-seeds 64 --arm-equal --m0-conditioning "" --m0-sparsity results/m0c/sparsity.json --conditioning-out results/m0c/conditioning.json --out results/m0c/reproduce.json` (약 12분) — 기대: `arm_equal true`, `decide_equal true`, `sparsity_exact true`(풀 = 인프로세스), 폭주 `seeds_with_kc_over_sat 0`.
...
6. `uv run python scripts/write_m0c_summary.py` → `results/summary/m0c.json`, 한 줄 판정. `uv run pytest -q tests/test_summary.py` 초록 확인.
7. `results/m0c/gate_runs.md`: 이번 세션에서 시험한 변형 전부(스펙 D.2의 표와 문단: KC→KC 제거, 클립 하한 1.0, 전 KC 총 흥분 정규화, PN 없는 KC 폴백, 스케일 0.5/0.25, 반구 보정 끔)를 표로 남긴다. 저장소에는 `results/`가 제외되므로 표의 사본을 스펙 D.8에 넣는다.
8. 스펙 **D.8 M0c 결과**: 게이트 표(항목별 예측 대 결과), 기저 8시드 평균 ± 표준편차와 시드별 값, 폭주(휴지 8시드, 냄새 B 64시드: 150 Hz 초과 0/64인지, 100 Hz 초과 수 분포, 최대 KC 발화율), 조건화 시드 8–15 n_flip·팔별 mean dD·채널별 특이 억제와 시드 0–7 보고, 등가성 셋, 처리량 표와 예산(옛 42.2시간 대비). 판정 한 줄.
9. README "측정된 것" M0c 항목(수치), M0b 항목 끝의 "STD 재검토 조건 충족" 문구 뒤에 "→ M0c에서 해소(스펙 D)" 추가.
10. 기저가 3–4 Hz 밖이거나 폭주가 0이 아니면: 파라미터를 조정하지 않는다. `tests/test_summary.py::test_m0c_summary_records_the_pre_registered_gate`는 `gate.passed`를 강제하지 않으므로 그대로 두고, D.8에 FAIL을 기록한 뒤 사용자에게 판정을 받는다(D.4). 조건화 기준은 예측대로 FAIL이어도 M0c 판정과 무관하다.
11. `docs/handoffs/2026-09-15-m2-handoff.md`의 STD 항목을 D 기준으로 갱신하고 M2 첫 측정 목록(D.6)을 적는다.
